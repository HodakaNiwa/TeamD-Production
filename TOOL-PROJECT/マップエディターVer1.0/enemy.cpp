...
#define ENEMY_AI_NONE_TIME       (10)    // 動きを止めてから動き出すまでの時間
...
void CEnemy::Uninit(void)
{
	...
	if (CManager::GetMode() == CManager::MODE_DEMOPLAY)
	{
		...
		if (m_bItem == true)
		{
			pDemoplay->CreateItem(GetPos(), INITIALIZE_D3DXVECTOR3, m_nItemType);
		}
	}
...
{
	if (GetAI_State() == AI_STATE_MOVE && m_nAI_MoveCounter % ENEMY_AI_MOVE_TIME == 0)
	{
