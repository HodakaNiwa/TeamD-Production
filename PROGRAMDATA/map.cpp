...
void CMap::FieldDown(D3DXVECTOR3 pos, int nXBlock, int nZBlock, float fFieldDown)
{
	...
	int nXArea = (int)((pos.x + (MASU_SIZE_X * MASU_BLOCK_X / 2)) / MASU_SIZE_X) + 1;
	int nZArea = (int)((-pos.z + (MASU_SIZE_Z * MASU_BLOCK_Z / 2)) / MASU_SIZE_Z) + 1;
	int nStartLeftVertex = (nXArea - 1) + ((MASU_BLOCK_X + 1) * (nZArea - 1));

	// 分割数を考慮して戻す
	nStartLeftVertex -= (nXBlock / 2);
	nStartLeftVertex -= ((nZBlock / 2)) * (MASU_BLOCK_X + 1);
	...
	{// 縦の分割数+1分繰り返し
		...
		nStartLeftVertex += (MASU_BLOCK_X + 1);
	}
