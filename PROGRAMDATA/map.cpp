...
HRESULT CMap::Save(char *pSaveFileName)
{
	...
	// ゲームフィールド情報を保存
	SaveGameField(AUTOSAVEFILENAME_GAMEFIELD);

	// モデルリスト情報を保存
	SaveModel(AUTOSAVEFILENAME_MODELLIST);

	// テクスチャリスト情報を保存
	SaveTexture(AUTOSAVEFILENAME_TEXLIST);

	// ライト情報を保存
	SaveLight(AUTOSAVEFILENAME_LIGHT);

	// 配置物情報を保存
	SaveObject(AUTOSAVEFILENAME_OBJECT);
...
HRESULT CMap::ChangeMap(char *pLoadFileName)
{
	...
	// 新たにマップデータを読み込む
	strcpy(pLoadFileName, "\0");       // ファイル名コピー
...
HRESULT CMap::SaveModelList(CFileSaver *pFileSaver)
{
	...
	pFileSaver->Print("%s%s\n\n", MODELLIST_FILENAME, m_aModelListFileName);
...
HRESULT CMap::SaveTexList(CFileSaver *pFileSaver)
{
	...
	pFileSaver->Print("%s%s\n\n", TEXLIST_FILENAME, m_aTexListFileName);
...
HRESULT CMap::SaveLight(char *pLightFileName)
{
	...
	{// ファイルが読み込めた
		...
		pFileSaver->Print("#------------------------------------------------------------------------------\n");
		pFileSaver->Print("%s%d\n\n", NUM_LIGHT, m_pLightManager->GetNumLight());
...
//    モデルリストを破棄する処理
//=============================================================================
void CMap::DeleteModelList(void)
{
	// モデル管轄クラスを破棄する
