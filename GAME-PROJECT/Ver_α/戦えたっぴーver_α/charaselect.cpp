...
void CCharaSelect::CreatePlayer(void)
{
	...
	for (int nCntPlayer = 0; nCntPlayer < MAX_NUM_PLAYER; nCntPlayer++)
	{
		...
		{
			m_pPlayer[nCntPlayer][nCntType] = CScene3D::Create(D3DXVECTOR3(sinf(m_fSelectRot[nCntPlayer] + fAddRot) * CHARASELECT_CIRCLE_RADIUS, 0.0f, cosf(m_fSelectRot[nCntPlayer] + fAddRot) * CHARASELECT_CIRCLE_RADIUS), D3DXVECTOR3(D3DX_PI * 0.5f, 0.0f, 0.0f), D3DXCOLOR(1.0f, 1.0f, 1.0f, 1.0f), 30.0f, 30.0f);
...
void CCharaSelect::CreatePlayerDataPointer(void)
{
	...
	for (int nCntPlayer = 0; nCntPlayer < MAX_NUM_PLAYER; nCntPlayer++)
	{
		m_pPlayer[nCntPlayer] = new CScene3D*[m_nNumPlayerData];
		for (int nCntType = 0; nCntType < m_nNumPlayerData; nCntType++)
		{
...
//=============================================================================
void CCharaSelect::WaitPartnerUpdate(void)
{
...
//=============================================================================
// キャラセレクトの次のモードに行くための入力待機処理
//=============================================================================
void CCharaSelect::WaitInputToNextMode(void)
{
	CInputKeyboard *pKey = CManager::GetKeyboard();
	CFade *pFade = CManager::GetFade();
	if (pKey == NULL || pFade == NULL) return;
...
//=============================================================================
// キャラセレクトの現在選択しているプレイヤー番号を設定する
//=============================================================================
void CCharaSelect::SetSelectPlayer(const int nSelectPlayer, const int nIdx)
{
	m_nSelectPlayer[nIdx] = nSelectPlayer;
}
...
//=============================================================================
// キャラセレクトの現在選択しているプレイヤー番号を取得する
//=============================================================================
int CCharaSelect::GetSelectPlayer(const int nIdx)
{
